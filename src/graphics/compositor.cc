...
bool compare(const std::shared_ptr<NWindow>& a,
             const std::shared_ptr<NWindow>& b) {
  if (a->getZindex() == b->getZindex()) return a < b;
...
Compositor& Compositor::instance() {
  if (!_instance) _instance = std::unique_ptr<Compositor>(new Compositor());
  return *_instance;
}
...
NWindow* Compositor::getActiveWindow() { return (*activeWindow).get(); }
...
void Compositor::update() { doupdate(); }
    ...
    if (!handled) {
      switch (ch) {
        ...
        case 27:
          ...
          break;
        case ']':
          if (activeWindow != (--windows.end())) ++activeWindow;
          refresh();
          break;
        case '[':
          if (activeWindow != windows.begin()) --activeWindow;
          refresh();
          break;
        case '0':
        case '1':
...
void Compositor::bindWindow(shared_ptr<NWindow>& window,
                            shared_ptr<NWindow> instance, bool setActive) {
  if (window) this->removeWindow(window);
  window = instance;
